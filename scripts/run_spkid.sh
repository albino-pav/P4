...
for cmd in $*; do
   ...
   if [[ $cmd == train ]]; then
       ...
       gmm_train -i 1 -v 5 -T 0.000001 -N 200 -m 20 -d $w/$FEAT -e $FEAT -g $w/gmm/$FEAT/$world.gmm $lists/verif/$world.train || exit 1
 #modificar esta linea para que baje el cost detection 20 mas o menos
