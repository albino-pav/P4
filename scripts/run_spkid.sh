...
# \DONE Creadas variables usadas a lo largo del script. Añadida variable para el test final
lists=lists
w=work
name_exp=one
db=spk_8mu/speecon
...
# \DONE compute_mfcc() y compute_lpcc() implementadas
compute_lp() {
...
for cmd in $*; do
   ...
   if [[ $cmd == train ]]; then
       ...
       # \DONE Parámetros del gmm_train escogidos para obtener los mejores resultados
       ...
       # \DONE finalverif implementado
        ...
        # cambiar el umbral por el optimo con el sistema bien entrenado
