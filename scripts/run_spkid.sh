...
compute_lpcc() {
    for filename in $(cat $lists/class/all.train $lists/class/all.test); do
        mkdir -p `dirname $w/$FEAT/$filename.$FEAT`
        EXEC="wav2lpcc 12 15 $db/$filename.wav $w/$FEAT/$filename.$FEAT"
        echo $EXEC && $EXEC || exit 1
    done
}
...
for cmd in $*; do
   ...
   if [[ $cmd == train ]]; then
       ...
       for dir in $db/BLOCK*/SES* ; do
           name=${dir/*\/}
           echo $name ----
           gmm_train  -v 1 -T 0.001 -i 1 -N 60 -m 64 -d $w/$FEAT -e $FEAT -g $w/gmm/$FEAT/$name.gmm $lists/class/$name.train || exit 1
           echo
       done
   elif [[ $cmd == test ]]; then
       ...
       gmm_train  -v 1 -T 0.001 -i 1 -N 35 -m 64 -d $w/$FEAT -e $FEAT -g $w/gmm/$FEAT/$world.gmm $lists/verif/$world.train || exit 1
	   ...
	   # lists/final/verif.test.candidates
        ...
        if ($F[2] >  2.2117092796004) {print "1\n"}  
        else {print "0\n"}' tee $w/final_verif_${FEAT}_${name_exp}.log | tee verif_test.log
        # cambiar el umbral 2.211 por el optimo con el sistema bien entrenado
