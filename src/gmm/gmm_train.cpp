...
int read_options(int ArgC, const char *ArgV[], Directory &input_dir,
                 Ext &input_ext, vector<string> &filenames, unsigned int &nmix,
                 string &gmm_filename, unsigned int &init_iterations,
                 unsigned int &em_iterations, float &init_threshold,
                 float &em_threshold, int &init_method, unsigned int &verbose);
...
int main(int argc, const char *argv[]) {
  ...
  int retv = read_options(argc, argv, input_dir, input_ext, filenames, nmix,
                          gmm_filename, init_iterations, em_iterations,
                          init_threshold, em_threshold, init_method, verbose);
  if (retv != 0)
  ...
  /// \TODO Initialize GMM from data; initially, you should implement random
  /// initialization.
  ///
  /// Other alternatives are: vq, em_split... See the options of the program and
  /// place each initicialization accordingly.
  switch (init_method) {
  case 0: // inicialiciación aleatoria
    gmm.random_init(data, nmix);
  case 1: // inicialiciación vq
    // vols que t'inicialitzi la fdp d'un locutor concret estigui feta amb
    // nmix gaussianes. Li passes les 5 senyals d'un locutor
    // data: param de les 5 senyals d'un locutor
    // init_threshold -> hem de mirar nosaltres on va bé
  ...
  /// \TODO Apply EM to estimate GMM parameters (complete the funcion in
  /// gmm.cpp)
...
int usage(const char *progname, int err) {
  ...
  cerr << "Options can be: \n"
       ...
       << ")\n"
       << "  -T thr\tLogProbability threshold of final EM iterations (def. "
       << DEF_THR << ")\n"
       << "  -i init\tInitialization method: 0=random, 1=VQ, 2=EM split (def. "
          "0)\n"
       << "  -v int\tBit code to control \"verbosity\" (eg: 5 => 00000101"
       << ")\n";
  cerr << "\nIn case you use initialization by VQ or EM split, the following "
          "options also apply: \n"
       << "  -n ite\tNumber of iterations in the initialization of the GMM "
          "(def. "
       << DEF_ITERATIONS << ")\n"
       << "  -t thr\tLogProbability threshold for the EM iterations in "
          "initialization (def. "
       << DEF_THR << ")\n";
  return err;
}
...
int read_options(int ArgC, const char *ArgV[], Directory &input_dir,
  ...
  // optarg and optind are global variables declared and set by the getopt()
  // function

  while ((option = getopt(ArgC, (char **)ArgV, "d:e:m:g:n:N:t:T:i:v:F")) !=
         -1) {
    switch (option) {
    case 'd':
      input_dir = optarg;
