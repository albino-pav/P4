...
float verify(const GMM &gmm_candidate, const GMM & gmm_world, const fmatrix &dat,
  ...
  //DONE: Verificación hecha teneiendo en cuenta modelo del mundo
  float score = 0.0F;
  lprobcand = gmm_candidate.logprob(dat);
  lprobbackground = gmm_world.logprob(dat);
  
  score = lprobcand - lprobbackground; // Normalizamos el score respecto el modelo genera: Modelo del mundo
